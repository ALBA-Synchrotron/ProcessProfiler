...
__doc__ = """
    ...
    A sample code to do so would be::
        ...
        import fandango
        host = fandango.linos.MyMachine().host
        #Create the device
        fandango.tango.add_new_device('ProcessProfiler/%s'%host,'ProcessProfiler','sys/profile/%s'%host)
        #Setup properties
        fandango.get_database().put_device_property('sys/profile/%s'%host,{'ProcessList':['Starter','notifd','ProcessProfiler']})
        #Start the device
        fandango.Astor('sys/profile/%s'%host).start_servers(host=host)
        #Start the update() command polling
        time.sleep(15.)
        fandango.get_device(sys/profile/%s'%host).poll_command('update',10000)
    """

#def get_nthreads(process_pid):
#    comm = "ps -ef |grep %d|grep -v grep |awk '{print $4}'"%process_pid
#    threads_list = fandango.linos.shell_command(comm).split('\n')
...
#    cpu = fandango.linos.shell_command(comm)
#    return float(cpu)
#def get_memory(pid):
...
#def get_threads_cpu_and_memory(pid):
    #try:
        ...
        #bar = filter(bool,fandango.linos.shell_command(comm).strip().split(' '))
...
#def get_processes_pid(process_name):
    #try:
        ...
        #process_list = fandango.linos.shell_command(comm)
...
def get_all_process(regexp=''):
    ...
    try:
        ...
        lines = fandango.linos.shell_command(comm).split('\n')
        ...
        for process in self.ProcessList:
            ...
            for suffix,data_type,data_format,unit in\
                ...
                if data_format == PyTango.SCALAR:
                    ...
                    if self.UseEvents: self.set_change_event(aname+'_'+suffix, True, False)
...
#------------------------------------------------------------------
    def __init__(self,cl, name):
        ...
        #self.log = fandango.Logger('ProcessProfiler/%s'%name)
...
#------------------------------------------------------------------
    def init_device(self):
        ...
        if self.UseEvents:
            ...
            self.set_change_event('LoadAverage', True, False)
